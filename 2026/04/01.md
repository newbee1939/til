...
Anthropicの AIコーディングツール「Claude Code」の npm パッケージ（`@anthropic-ai/claude-code` v2.1.88）に、**ソースマップ（.mapファイル）が含まれたまま公開**されてしまった。1,902ファイル・51万行超の TypeScript ソースコードが誰でも読める状態に。ハッキングではなく、**ビルド設定のミス**。
