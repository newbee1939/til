...
4. `npm install` すると `plain-crypto-js` の `postinstall` スクリプトが自動実行され、OS ごとにRAT（遠隔操作マルウェア）をダウンロード・配置
...
CI/CDでは `npm install` ではなく `npm ci` を使う。`npm ci` はロックファイルと不整合があればエラーで止まり、未知のバージョンを勝手に取得しない。
